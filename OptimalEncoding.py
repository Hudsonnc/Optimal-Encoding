...
class OptimalEncoding_Weight_Noise(object):
    def __init__(self, encoder, decoder, k):
        #Encoder n_out must equal k
        self.encoder = encoder
        #Decoder n_in must equal k
        ...
        #placeholder for epsilon
        self.epsilon = [
            tf.placeholder(tf.float32, shape=(None, width), name = 'epsilon')
            for width in encoder.widths
        ]
        
        #Encode
        self.Z = encoder(self.X, self.epsilon, linear_out=True)
        ...
        with sess.as_default():
            ...
            for epoch in tqdm(range(epochs)):        
                ...
                for batch in range(n_batches):
                    ...
                    epsilon = [
                        np.random.normal(0,1, size=(batch_size, eps.shape[-1]))
